...
def _openclaw_agent_prompt(request_payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "delta, retrieval sufficiency, or forecast writes unless a strict anchor "
        "dependency is validated. When AMRG hints are relevant, consume them only by "
        "copying their hint_ref into branch or leaf amrg_usage_refs; leave irrelevant "
        "hints unreferenced so the operator report can emit ignored reason codes. "
