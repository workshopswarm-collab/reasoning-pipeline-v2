...
- Status: production-readiness mode is allowed; scoreable live operations remain blocked.
...
- Scoreable live scheduler runs must pass `run_ads_operational_scheduler.py --require-live-readiness --require-scoreable-live`.

## Blocked Until Further Evidence

- Scoreable live prediction writes from non-canary handlers.
...
- Calibration-debt production as a scoreable live path until CAL-001 clears.

## Live Cutover Gate

Scoreable live operations require all of the following:

- Real specialist adapters for live retrieval, source metadata classification, researcher classification, verification, and SCAE-ready sufficiency reconciliation.
